...
public class CList <E> {
	...
	public void insert(E newItem) { // last가 가리키는 노드의 다음에 새노드 삽입
		...
		if (last == null) { //리스트가 empty 일때
			newNode.setNext(newNode);
			last = newNode;
		}
		else {
			newNode.setNext(last.getNext()); // newNode 의 다음 노드가
											 //last 가 가리키는 노드의 다음노드가 되도록
			last.setNext(newNode); // last 가 가리키는 노드의 다음 노드가 newNode 가 되도록
		}
		size++;
	}
	...
	public Node delete() { // last가 가리키는 노드의 다음 노드를 제거
		...
		if (x == last) last = null; //리스트에 1 개의 노드만 있는 경우
		else {
			last.setNext(x.getNext()); // last가 가리키는 노드의 다음 노드가 x 의 다음 노드가 되도록
			x.setNext(null); // x의 next 를 null 로 만든다
		}
		size--;
		return x;
	}
