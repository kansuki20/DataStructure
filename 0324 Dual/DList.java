...
public class DList <E> {
	...
	public DList(){ //생성자
		...
		tail = new DNode (null, head, null); // tail의 이전 노드를 head 로 만든다
		head.setNext(tail); //head의 다음 노드를 tail 로 만든다
		size = 0;
	}
	...
	public void delete(DNode x) { // x가 가리키는 노드 삭제
		...
		f.setNext(r);
		r.setPrevious(f);
		x.setPrevious(null); // x의 previous 를 null 로 만든다
		x.setNext(null); // 의 next 를 null 로 만든다
		size--;
	}
	
	public void print(){ //연결 리스트 노드들의 item 들을 차례로 출력
		if (size > 0)
